...
(function(MM) {
    ...
    MM.MouseHandler.prototype = {
        ...
        teardown: function(map) {
            MM.removeEvent(map.parent, 'dblclick', this.getDoubleClick());
            MM.removeEvent(map.parent, 'mousedown', this.getMouseDown());
            // MM.removeEvent(map.parent, 'mousewheel', this.getMouseWheel());            
            this.map = null;
        },
    ...
    */    
    ...
    MM.Map.prototype = {
        ...
        zoomBy: function(zoomOffset) {	
			if(this.checkZoomRange(zoomOffset)){
		...
		//	checks if zoom falls with min/max zoom range
		checkZoomRange: function(zOffset){
			var newIntendedZoom = this.getZoom() + zOffset;
			var r = true;
			if(this.minZoom != null && zOffset < 0 ){
				r = (newIntendedZoom >= this.minZoom) ?  true : false;
			}else if(this.maxZoom != null && zOffset > 0){
				r = (newIntendedZoom <= this.maxZoom) ?  true : false;
			}
			return r;
		},
		...
		setMaxZoom: function(n){
			this.maxZoom = Math.floor( Number(n) );
		},
		setMinZoom: function(n){
			this.minZoom = Math.floor( Number(n) );
		},
        ...
        requestRedraw: function() {
            ...
            if (!this._redrawTimer) {
                this._redrawTimer = setTimeout(this.getRedraw(), 10);
            }
        },
